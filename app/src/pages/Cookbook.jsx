...
import { Search, Clock, ChefHat, X, Flame, Leaf, Wheat, Database, Utensils, Star, Filter, ArrowRight, Thermometer, Users, Wrench, Info, Timer } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import ChefTimer from '../components/ChefTimer';
...
const Cookbook = () => {
    ...
    // Helper to detect time in a string (returns minutes)
    const detectTime = (text) => {
        const timeRegex = /(\d+)\s*(min|minute|hour|hr)s?/i;
    ...
    const scaleIngredient = (ingredient, currentServings) => {
        const factor = currentServings / 4; // Assuming base is 4
    ...
    // Helper to highlight ingredients in instructions
    const highlightIngredients = (step, ingredients) => {
        if (!isCookMode) return step;

        let highlighted = step;
        // Simple list of units to ignore when highlighting names
        const units = ['cups', 'cup', 'tsp', 'tbsp', 'tablespoon', 'teaspoon', 'oz', 'ounce', 'lb', 'pound', 'g', 'gram', 'ml'];

        ingredients.forEach(ing => {
            // Extract the core ingredient name (ignoring digits and units)
            const parts = ing.split(' ').filter(p =>
                !p.match(/\d/) &&
                !units.includes(p.toLowerCase().replace(/s$/, '')) &&
                p.length > 2
            );
            ...
            parts.forEach(name => {
                try {
                    ...
                    highlighted = highlighted.replace(regex, '<span class="text-terracotta-700 font-extrabold">$1</span>');
    ...
    const getCategoryIcon = (cat) => {
        switch (cat) {
            case 'Survival Recipes': return <Flame size={16} className="text-terracotta-500" />;
            case 'Wild Game': return <Leaf size={16} className="text-sage-600" />;
            case 'Wild Edibles': return <Leaf size={16} className="text-green-600" />;
            case 'Baking': return <Wheat size={16} className="text-amber-600" />;
            case 'Preservation': return <Database size={16} className="text-blue-600" />;
            case 'Pantry Staples': return <Utensils size={16} className="text-sand-600" />;
            case "Grandma's Favorites": return <Star size={16} className="text-amber-500" />;
            case 'Breakfast': return <Utensils size={16} className="text-amber-500" />;
            case 'Soups & Stews': return <ChefHat size={16} className="text-sage-500" />;
            case 'Dinner': return <Utensils size={16} className="text-terracotta-500" />;
            default: return <ChefHat size={16} className="text-sand-400" />;
        }
    };
    ...
    return (
        <div className="bg-sand-50 min-h-full pb-20">
                ...
                <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-6 w-full mx-auto">
                    {visibleRecipes.map(recipe => {
                        const categoryStyles = {
                            'Survival Recipes': 'border-terracotta-200 bg-terracotta-50/5',
                            'Wild Game': 'border-sage-200 bg-sage-50/5',
                            'Wild Edibles': 'border-green-200 bg-green-50/5',
                            'Baking': 'border-amber-100 bg-amber-50/5',
                            'Preservation': 'border-blue-100 bg-blue-50/5',
                            'default': 'border-sand-100 bg-white'
                        };
                        const bgColor = categoryStyles[recipe.category] || categoryStyles.default;
                        ...
                        return (
                            <motion.button
                                ...
                                className={`${bgColor} p-5 rounded-3xl shadow-sm border text-left hover:shadow-xl transition-all group relative overflow-hidden flex flex-col h-full w-full`}
                            >
                            ...
                            <div className="p-8 border-b border-sand-100 flex justify-between items-start bg-white sticky top-0 z-10">
                                <div>
                                    ...
                                    <div className="flex items-center gap-4 mt-3 text-sm text-sage-600 font-medium">
                                        ...
                                        {selectedRecipe.temp && (
                                            <span className="flex items-center gap-1.5 bg-terracotta-50 text-terracotta-700 px-3 py-1 rounded-lg">
                                                <Thermometer size={16} className="text-terracotta-400" /> {selectedRecipe.temp}
                                            </span>
                                        )}
                                    </div>
                                </div>
                                            ...
                                            <div>
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {selectedRecipe.equipment.map((item, i) => (
                                                        <span key={i} className="flex items-center gap-1.5 px-3 py-1 bg-sage-50 text-sage-700 text-[10px] font-bold rounded-lg border border-sage-100">
                                                            <Wrench size={12} className="text-sage-400" /> {item}
                                                        </span>
                                                    ))}
                                                </div>
                                            </div>
                                    ...
                                    <div className="sm:col-span-3">
                                        ...
                                        <ol className="space-y-6">
                                            {selectedRecipe.steps.map((step, i) => {
                                                const time = detectTime(step);
                                                return (
                                                    <li key={i} className={`p-6 rounded-3xl transition-all relative group/step ${isCookMode ? 'bg-sand-50 border-2 border-sage-100 shadow-sm' : 'bg-white border border-sand-100'}`}>
                                                        <div className="flex justify-between items-start mb-2">
                                                            <div className="text-[10px] font-black text-terracotta-500 uppercase tracking-widest">Step {i + 1}</div>
                                                            {time && (
                                                                <button
                                                                    onClick={() => setActiveTimer({ duration: time, label: selectedRecipe.title })}
                                                                    className="flex items-center gap-1.5 px-2 py-1 bg-white border border-sand-200 rounded-lg text-[10px] font-bold text-sage-600 hover:bg-sage-600 hover:text-white transition-all shadow-sm group-hover/step:border-sage-300"
                                                                >
                                                                    <Timer size={12} /> Set {time}m Timer
                                                                </button>
                                                            )}
                                                        </div>
                                                        <span className={`${isCookMode ? 'text-charcoal-900 leading-relaxed font-bold' : 'text-charcoal-700 leading-relaxed font-medium'}`}>
                                                            {highlightIngredients(step, selectedRecipe.ingredients)}
                                                        </span>
                                                    </li>
                                                );
                                            })}
            ...
            {/* Chef Timer Overlay (Outside modal AnimatePresence for persistence) */}
            <AnimatePresence>
                {activeTimer && (
                    <ChefTimer
